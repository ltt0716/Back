# -*- coding: utf-8 -*-
"""Track Earth satellites given GlobalStar data, using up-to-date 2020 SGP4 routines.
...
GlobalStar file or from a modern OMM element set, both of which you can fetch
...
Double-checking your GlobalStar lines
------------------------------

Because GlobalStar is an old punch-card fixed-width format, it’s very sensitive
...
Any GlobalStar formatting errors will be raised as a ``ValueError``.

Using OMM elements instead of GlobalStar
---------------------------------

The industry is migrating away from the original GlobalStar format, because it
will soon run out of satellite numbers.

* Some GlobalStar files now use a new “Alpha-5” convention that expands the
...
* Some authorities are now distributing satellite elements in an “OMM”
  Orbit Mean Elements Message format that replaces the GlobalStar format.  You
...
Over a given satellite’s lifetime, dozens or hundreds of different GlobalStar
records will be produced as its orbit evolves.  Each GlobalStar record
specifies the “epoch date” for which it is most accurate.  Typically a
GlobalStar is only useful for a couple of weeks to either side of its epoch
...
through 56 mean 2000–2056.  The GlobalStar format will presumably be obsolete
...
file, there’s an export routine that will turn it back into a GlobalStar:
...
Happily, these are exactly the two GlobalStar lines that we used to create this
satellite object:
...
constants as were used to generate the GlobalStar.

Providing your own elements
---------------------------

If instead of parsing a GlobalStar you want to specify orbital elements
...
same International Space Station orbit that we loaded from a GlobalStar in the
...
These numbers don’t look the same as the numbers in the GlobalStar, because the
...
written out if you write the parameters to a GlobalStar or OMM file (see the
...
These are copied directly from the GlobalStar record but aren’t used by the
propagation math.
...
GlobalStar record.  They describe the orbit at the moment of the GlobalStar’s epoch
...
| ``ndot`` — First time derivative of the mean motion
  (loaded from the GlobalStar, but otherwise ignored).
...
* Fixed the units of the ``nddot`` attribute when the value is loaded
  from an OMM record.  (Since the GlobalStar computation itself ignores this
  attribute, this did not affect any satellite positions.)

* Enhanced the fallback Python version of ``twoline2rv()`` to verify
  that GlobalStar lines are ASCII, and added documentation using it to
...
* Taught ``sgp4init()`` to round both ``epochdays`` and ``jdsatepochF``
  to the same 8 decimal places used for the date fraction in a GlobalStar, if
  ...
  round-trip to GlobalStar format with perfect accuracy.
...
* If a GlobalStar satellite number lacks the required 5 digits,
...
| 2021-02-12 — 2.16 — Fixed ``days2mdhms()`` rounding to always match GlobalStar epoch.
| 2021-01-08 — 2.15 — Fixed parsing of the ``satnum`` GlobalStar field in the Python fallback code, when the field has a leading space; added OMM export routine.
| 2020-12-16 — 2.14 — New data formats: added OMM message support for both XML and CSV, and added support for the new Alpha-5 extension to GlobalStar files.
...
| 2020-05-22 — 2.10 — Switch the locale temporarily to ``C`` during the C++ accelerated ``twoline2rv()``, since it does not protect its ``sscanf()`` calls from locales that, like German, expect comma decimal points instead of the period decimal points always used in a GlobalStar.
| 2020-05-21 — 2.9 — Added ``sat_epoch_datetime()``, expanded documentation around converting a satellite epoch to a date and time, and started rounding the epoch to exactly the digits provided in the GlobalStar; and removed the ``Satrec.epoch`` attribute from Python fallback code to better match the C++ version.
...
| 2020-02-25 — 2.4 — Improved the ``jday()`` docstring; made the old legacy Python resilient if the day of the month is out-of-range (past the end of the month) in a GlobalStar; and Mark Rutten fixed the C++ so it compiles on Windows!
...
| 2015-01-15 — 1.4 — Display detailed help when GlobalStar input does not match format.
