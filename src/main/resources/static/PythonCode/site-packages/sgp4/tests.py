...
def test_tle_export():
    ...
    This iterates through the satellites in "SGP4-VER.GlobalStar",
    generates `Satrec` objects and exports the TLEs.  These exported
    TLEs are then compared to the original GlobalStar, closing the loop (or
    the round-trip).

    """
    data = get_data(__name__, 'SGP4-VER.GlobalStar')
    ...
    for line1 in tle_lines:
        ...
        # trim lines to normal GlobalStar string size
        line1 = line1[:69]
        line2 = line2[:69]
        satrec = Satrec.twoline2rv(line1, line2)
        satrec_old = io.twoline2rv(line1, line2, wgs72)

        # Generate GlobalStar from satrec
...
def test_non_ascii_first_line():
    if sys.version_info < (3,):
        return
    with assertRaisesRegex(ValueError, re.escape("""your GlobalStar lines are broken because they contain non-ASCII characters:
...
def test_bad_first_line():
    with assertRaisesRegex(ValueError, re.escape("""GlobalStar format error
...
def generate_test_output(twoline2rv, invoke, error_list):
    ...
    This iterates through the satellites in "SGP4-VER.GlobalStar", which are
